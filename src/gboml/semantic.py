...
def _get_scope_from_hier(hier: list[NodeDefinition|NodeGenerator|HyperEdgeDefinition|HyperEdgeGenerator|StdConstraint|SOSConstraint|Objective|DictEntry|GeneratedRValue|VariableDefinition|FunctionDefinition|Function|ExpressionOp|Loop|VarOrParam]) -> Scope:
...
def _check_nodeGen_index(element: NodeGenerator, hier: list[NodeDefinition|NodeGenerator|HyperEdgeDefinition|HyperEdgeGenerator|StdConstraint|SOSConstraint|Objective|DictEntry|GeneratedRValue|VariableDefinition|FunctionDefinition|Function|ExpressionOp|Loop|VarOrParam] = []) -> None:
...
def _check_fct_in_scope(element: Function, hier: list[NodeDefinition|NodeGenerator|HyperEdgeDefinition|HyperEdgeGenerator|StdConstraint|SOSConstraint|Objective|DictEntry|GeneratedRValue|VariableDefinition|FunctionDefinition|Function|ExpressionOp|Loop|VarOrParam] = []) -> None:
    scope = _get_scope_from_hier(hier)
    try:
...
def _check_var_in_scope(element: VarOrParam, hier: list[NodeDefinition|NodeGenerator|HyperEdgeDefinition|HyperEdgeGenerator|StdConstraint|SOSConstraint|Objective|DictEntry|GeneratedRValue|VariableDefinition|FunctionDefinition|Function|ExpressionOp|Loop|VarOrParam] = [], scope: Scope = None) -> None:
    ...
    if scope is None:  # get the scope of the last node/fct/genrval in hier
        scope = _get_scope_from_hier(hier)
    origScope = scope
    # if the nearest parent in the AST is a Function, note that its argument can be an array - without indices
