...
and passes it to the dsp solver.
  ...
  Typical usage example:
   ...
   solution, objective, status, solver_info = dsp_solver(matrix_a, vector_b, vector_c,
...
from solver_api.dsppy import DSPpy


def dsp_solver(matrix_a: coo_matrix, vector_b: np.ndarray, vector_c: np.ndarray,
               objective_offset: float, name_tuples: dict, structure_indexes, algorithm="de") -> tuple:
    """dsp_solver

        takes as input the matrix A, the vectors b and c. It returns the solution
        of the problem : min c^T * x s.t. A * x <= b found by the dsp solver
        ...
        Args:
            ...
            structure_indexes -> constraint indexes of the different blocks (last one being the master block)
            algorithm -> solving algorithm to use, either "de" for the extensive form and "dw" for Dantzig-Wolf
    ...
    dsp.loadBlockProblem(pointer_to_model, 0, nb_cols, master_nb_lines, len(master_val),
                         master_row, master_col, master_val, minus_infinity_col, plus_infinity_col, master_coltypes,
    ...
    for b_number in range(nb_blocks-1):
        ...
        block_csr = all_constraints_matrix[block_start_constr:block_end_constr + 1, 0:nb_cols]
        ...
        dsp.loadBlockProblem(pointer_to_model, b_number + 1, nb_cols, block_nb_lines, len(block_val),
                             block_row, block_col, block_val, minus_infinity_col, plus_infinity_col, master_coltypes,
                             vector_c, row_lb, row_up)
