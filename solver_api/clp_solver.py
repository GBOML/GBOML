...
and passes it to the CLP/CBC solver.
  ...
  Typical usage example:
   ...
   solution, objective, status, solver_info = clp_solver(matrix_a, vector_b, vector_c,
...
def clp_solver(matrix_a: coo_matrix, vector_b: np.ndarray, vector_c: np.ndarray, objective_offset: float,
               name_tuples: list) -> tuple:
    """clp_solver

        takes as input the matrix A, the vectors b and c. It returns the solution
        of the problem : min c^T * x s.t. A * x <= b found by the clp/cbc solver
    ...
    solver_info = {"name": "clp", "algorithm": "primal simplex", "status": cbc_model.status}
    solution = None
    objective = None
