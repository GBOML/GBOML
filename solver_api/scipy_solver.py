...
and passes it to the linprog solver.
  ...
  Typical usage example:
   ...
   solution, objective, status, solver_info = scipy_solver(matrix_a, vector_b, vector_c,
...
def scipy_solver(matrix_a: coo_matrix, vector_b: np.ndarray, vector_c: np.ndarray,
                 objective_offset: float, name_tuples: dict) -> tuple:
    """scipy_solver

        takes as input the matrix A, the vectors b and c. It returns the solution
        of the problem : min c^T * x s.t. A * x <= b found by the scipy's linprog solver
