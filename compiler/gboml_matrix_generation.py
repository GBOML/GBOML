...
def get_flat_nodes_edges_ordered(nodes, hyperedges, without_hyperedges=False):
    ...
    for node in nodes:
        ...
        all_sub_nodes_edges = get_flat_nodes_edges_ordered(sub_nodes, sub_edges, without_hyperedges)
...
def get_flat_nodes_edges_and_definition(nodes, hyperedges, definitions, accumulator_dict):
    ...
    for node in nodes:
        ...
        sub_nodes = node.get_sub_nodes()
        sub_edges = node.get_sub_hyperedges()
        accumulator_dict[node_name] = get_only_objects_in_nested_dict_layer(definitions[node_name])
        node_definition_tuples.append([node, accumulator_dict.copy()])
        sub_node_tuples, sub_edge_tuples = get_flat_nodes_edges_and_definition(sub_nodes, sub_edges,
    ...
    for edge in hyperedges:
        ...
        accumulator_dict[edge_name] = get_only_objects_in_nested_dict_layer(definitions[edge_name])
...
def extend_factor_on_multiple_processes(root: Program, definitions, nb_processes) -> None:
    ...
    node_definition_tuples, edge_definition_tuples = get_flat_nodes_edges_and_definition(nodes, hyperlinks, definitions,
                                                                                         accumulator_dict)
    ...
    for node in nodes:
        ...
        for objective_index, objective_factor in enumerate(objective_factors_list):
            ...
            indep_terms[independent_terms_offset:
                        independent_terms_offset + number_of_independent_terms] = independent_terms
...
def matrix_generation_a_b(root: Program) -> tuple:
    ...
    nodes = root.get_nodes()
    hyperlinks = root.get_links()
    graph_elements = get_flat_nodes_edges_ordered(nodes, hyperlinks, without_hyperedges=False)
    length_values = 0
    length_independent_terms = 0
