...
def turn_to_dict(list_nodes: list) -> dict:
    ...
    for node in list_nodes:
        ...
        if name in dict_nodes:
            error_("The name : "+str(name) + " has already been used at line "+str(dict_nodes[name].get_line())
...
def update_branch_in_nested_dict(to_update_dict, list_keys, new_key, to_add_dict):
    if not list_keys:
        ...
        if first_key in to_update_dict:
            ...
            code, updated_branch = update_branch_in_nested_dict(next_level_dict, list_keys, new_key, to_add_dict)
            if code == -1:
        ...
        if key != list_keys[-1] or not not_lower:
            ...
            for nested_key in nested_dict.keys():
                if type(nested_dict[nested_key]) != dict:
                    intermediate_stopped_dict[nested_key] = nested_dict[nested_key]
