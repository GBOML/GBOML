...
class Time:
    ...
    def __init__(self, time_var: str, expr: Expression, line: int = None):
        ...
        if time_var != "T":
            ...
            error_("Semantic error:"+str(line)+": Use \"T\""+" as a symbol for the time horizon. \"" + str(time_var) +
                   "\" is not allowed")
        self.time = time_var
        self.expr = expr
...
class TimeInterval:
    ...
    def check(self, begin_value, end_value, step_value, clip: int = sys.maxsize) -> tuple:
    ...
    def convert_type(self, value, message: str = "") -> int:
        ...
        if type(value) == float and value.is_integer() is False:
            ...
            print("WARNING: in for loop, "+message+" value "+" is of type float and was rounded to "+str(value) +
