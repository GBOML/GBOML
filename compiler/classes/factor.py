...
def is_index_dependant_expression(expression,index):
    ...
    if e_type == 'literal':
        ...
        if type(seed)==Identifier:
            ...
            if id_type == "assign":
                return is_index_dependant_expression(identifier.get_expression(),index)
...
class Factorize:
    ...
    def compute_independant_term(self, expr, variables):
        ...
        if expr_type == "literal":
            ...
            if expr_type in ["*","/","**","mod"]:
                ...
                if var_1 and var_2:
                    is_var = True
                elif var_1 == False and var_2 == False:
                    expr_acc = Expression(expr_type)
                    expr_acc.add_child(expr1)
                    expr_acc.add_child(expr2)
                elif var_1: 
    ...
    def compute_factor(self,expr_acc, is_index_dependant,parent_expr, branch_considered,constants,stop_expr = None):
        ...
        if parent_expr == None or parent_expr == stop_expr or (self.type_fact == "sum" and parent_expr.get_type()=="sum"):
        ...
        if p_type == "*" or p_type == "/":
            ...
            if other_child_time_dependancy == False:
                term1 = child.evaluate_expression(constants)
                if is_index_dependant == False:
        ...
        return self.compute_factor(expr_acc, is_index_dependant, parent_expr, branch_considered,constants,stop_expr = None)
    ...
    def extend(self, definitions):
        ...
        if self.type_fact == "constraint":
            ...
            if (not is_index_dependant_expression(constraint.get_rhs(),name_index)) and (not is_index_dependant_expression(constraint.get_lhs(),name_index)):
                unique_evaluation = True
            
            for k in time_range:
                definitions[name_index]=[k]

                if constraint.check_time(definitions)==False:
                    continue
            ...
            for k in obj_range:
                ...
                if objective.check_time(definitions)==False:
                    continue
